...
// SaturationContrastFunction represents a function that adjusts the contrast of saturation values
...
// NewSaturationContrast creates a new saturation contrast adjustment function
func NewSaturationContrast(amount float64) *SaturationContrastFunction {
	return &SaturationContrastFunction{
		BaseFunction: fx.NewBaseFunction("satcontrast", "Applies saturation contrast transformation to an image", color.New(0, 0, 0, 1), satcontrastArgs),
		amount:       amount,
	}
}
...
func (f *SaturationContrastFunction) Apply(img *image.Image) (*image.Image, error) {
	...
	// Adjust saturation contrast for each pixel
	for y := bounds.Min.Y; y < bounds.Max.Y; y++ {
		for x := bounds.Min.X; x < bounds.Max.X; x++ {
			...
			// Adjust saturation contrast using the contrast curve
			s = contrastCurve(s, f.amount)
...
func (f *SaturationContrastFunction) ProcessPixel(x, y int, img *image.Image) (*color.Color64, error) {
	if img == nil || *img == nil {
		return nil, fx.ErrInvalidImage
	}
