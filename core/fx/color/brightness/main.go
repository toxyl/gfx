...
type BrightnessFunction struct {
	*fx.BaseFunction
	amount float64 // Brightness adjustment amount (-1.0 to 1.0)
}

// Function arguments
var brightnessArgs = []fx.FunctionArg{
	{
		Name:        "amount",
		Type:        fx.TypeFloat,
...
func NewBrightness(amount float64) *BrightnessFunction {
	return &BrightnessFunction{
		BaseFunction: fx.NewBaseFunction("brightness", "Applies brightness transformation to an image", color.New(0, 0, 0, 1), brightnessArgs),
		amount:       amount,
	}
}
...
// ProcessPixel applies the brightness effect to a single pixel
func (f *BrightnessFunction) ProcessPixel(x, y int, img image.Image) (*color.Color64, error) {
	if img == nil {
		return nil, fx.ErrInvalidImage
	}
	...
	// Adjust brightness by modifying luminance
...
func init() {
	fx.DefaultRegistry.Register(NewBrightness(0))
}
